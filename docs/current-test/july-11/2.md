...
O Sandbox Cloud Agent Group está disponível para testar os seus projetos com uma nova versão do Jitterbit Harmony para garantir que você esteja pronto(a) para a próxima atualização do Production Cloud Agent Group. **_A intenção não é que ele seja um ambiente de teste para os seus projetos antes do "go-live"_**—para isso use um [ambiente do Harmony](https://success.jitterbit.com/display/DOC/Environments?showLanguage=pt_BR) - como projetos no Sandbox Cloud Agent, caso eles usem um novo recurso introduzido naquela versão específica, eles não podem ser movidos para o Production Agent Group até que o Production Cloud Agent seja atualizado para a mesma versão.
