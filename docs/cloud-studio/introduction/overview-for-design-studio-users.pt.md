...
Se você usa o [Jitterbit Harmony Design Studio](/display/DOC/Design+Studio?showLanguage=pt_BR) (também chamado de
...
Harmony](/display/DOC/Jitterbit+Security+and+Architecture+White+Paper#JitterbitSecurityandArchitectureWhitePaper-MajorComponents?showLanguage=pt_BR),
que inclui os [Jitterbit Harmony Agents](/display/DOC/Agent?showLanguage=pt_BR) que executam os processos que
suportam integrações, o [Jitterbit Harmony Cloud Studio](/display/CS/Cloud+Studio?showLanguage=pt_BR) acelera o
...
Studio](/display/DOC/System+Requirements+for+Design+Studio?showLanguage=pt_BR).

Com o Cloud Studio, você não precisa mais instalar a aplicação de *design* na sua máquina. Em vez disso, o [Cloud
Studio](/display/CS/Cloud+Studio?showLanguage=pt_BR) oferece acesso de qualquer lugar que tenha conexão com a
Internet, independentemente de plataforma ou localização, através do [Harmony
Portal](/display/DOC/Jitterbit+Harmony+Portal?showLanguage=pt_BR). Os navegadores suportados incluem Chrome,
Firefox, e Safari (apenas para macOS). Além disso, você pode navegar diretamente de e para as outras aplicações
Harmony, tais como o [API Manager](/display/DOC/API+Manager?showLanguage=pt_BR), o
[Marketplace](/display/DOC/Marketplace?showLanguage=pt_BR) e o [Management
Console](/display/DOC/Management+Console?showLanguage=pt_BR) enquanto faz o *design* do seu projeto.
...
projetos](/display/CS/Project+Index?showLanguage=pt_BR), que mostra um repositório com todos os seus projetos do
...
Studio](/display/CS/Project+Exports+and+Imports?showLanguage=pt_BR). Os projetos do Cloud Studio estão no formato
de um arquivo JSON.

**Pergunta:** *Existem modelos para que eu não precise construir projetos do zero?*

**Resposta:** O [Jitterbit Marketplace](/display/DOC/Marketplace?showLanguage=pt_BR) oferece centenas de projetos
...
Studio](/display/CS/Cloud+Studio+Process+Templates?showLanguage=pt_BR), e sempre há mais deles sendo
...
*Template*](/display/DOC/Starting+a+Recipe+or+Template+Project?showLanguage=pt_BR). Um número limitado de
protótipos do [Citizen Integrator](/display/DOC/Citizen+Integrator?showLanguage=pt_BR), que são exclusivos para o
...
-   **Barra de ferramentas de projeto:** A [barra de ferramentas de
    projeto](/display/CS/Project+Toolbar?showLanguage=pt_BR) no topo permite que você acesse ações de projeto e
    navegue para projetos recentes.

-   **Painel de projeto:** O [painel de projeto](/display/CS/Project+Pane?showLanguage=pt_BR) à esquerda oferece
    navegação por todo o seu projeto.

-   **Design canvas:** O [design canvas](/display/CS/Design+Canvas?showLanguage=pt_BR) é o local de trabalho
    principal onde você projeta workflows.

-   **Paleta de componentes:** À direita, a [paleta de componentes de
    design](/display/CS/Design+Component+Palette?showLanguage=pt_BR) dá acesso a recursos de conectividade.

![](https://docs-source.jitterbit.com/cs/project/project-designer_annotated_pp.png)

Em vez de pastas, as operações são organizadas dentro de [workflows](/display/CS/Workflows?showLanguage=pt_BR) para
...
-   **Componentes:** Esta aba mostra todos os componentes que existem dentro do projeto. Como ao usar pastas, você
    ...
    customizados](/display/CS/Component+Groups?showLanguage=pt_BR).
...
organizações. Isto também nos permitiu criar o [Connector Builder](/display/CS/Connector+BuildershowLanguage=pt_BR),
...
*design*](/display/CS/Design+Component+Palette?showLanguage=pt_BR). Veja
[Conectores](/display/CS/Connectors?showLanguage=pt_BR) para conhecer todos os conectores disponíveis no Cloud
Studio.
...
No Cloud Studio, elas são chamadas de [ações de operação](/display/CS/Operation+Actions?showLanguage=pt_BR).
...
[ver uma prévia de uma *transformation*](/display/CS/Preview+Mode?showLanguage=pt_BR) e [criar mapeamentos
condicionais](/display/CS/Conditional+Mapping?showLanguage=pt_BR). Recursos adicionais que estão disponíveis somente
no Cloud Studio incluem [espelhar um *schema*](/display/CS/Mirrored+Schemas?showLanguage=pt_BR) de um lado da
*transformation* para outro, e a [exportação e importação de
mapeamentos](/display/CS/Transformation+Mapping+Exports+and+Imports?showLanguage=pt_BR) com a habilidade de
...
Script](/display/CS/Jitterbit+Script?showLanguage=pt_BR), com o mesmo suporte a
[JavaScript](/display/CS/JavaScript?showLanguage=pt_BR) que tem o Design Studio. No geral, os *scripts* são tratados
...
[Endpoints](/display/CS/Jitterbit+Script?showLanguage=pt_BR#JitterbitScript-connectors) no artigo [Jitterbit
Script](/display/CS/Jitterbit+Script?showLanguage=pt_BR)).
...
ou implantar o projeto inteiro (veja [Implantação de Projetos](/display/CS/Project+DeploymentshowLanguage=pt_BR)).
Projetos também podem ser migrados entre ambientes (veja [Migração de
Projetos](/display/CS/Project+Migration?showLanguage=pt_BR)).
